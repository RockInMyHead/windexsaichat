...
### 💬 AI Chat
- **GPT-4o Pro** - продвинутая модель для сложных задач
- **GPT-4o Mini** - быстрая модель для повседневных вопросов
...
```bash
git clone https://github.com/RockInMyHead/chat_windexai.git
cd chat_windexai
```
...
- Выбор между GPT-4o Mini (Lite) и GPT-4o (Pro)
...
### Чат с ИИ
1. **Выберите модель** - GPT-4o Mini или GPT-4o Pro
...
## 🔌 Доступ извне с помощью Ngrok

Если нужно дать доступ извне без правки сетевых настроек, воспользуйтесь ngrok:

```bash
# Убедитесь, что ngrok установлен: https://ngrok.com/
# Запустите скрипт для создания туннеля
./ngrok.sh
```

После запуска скрипта ngrok отобразит публичный URL вида `https://xxxxxx.ngrok.io`, по которому можно обратиться к приложению извне.
