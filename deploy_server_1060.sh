...
sshpass -p "$PASSWORD" scp -P "$PORT" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null windexai-project.tar.gz "$SERVER:$REMOTE_PATH/"
...
sshpass -p "$PASSWORD" ssh -p "$PORT" -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$SERVER" << 'EOF'
    cd ~/windexai-project
