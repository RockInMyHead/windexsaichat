...
from database import get_db, User, Deployment, SiteAnalytics
...
async def get_dashboard_overview(
    ...
    for deployment in deployments:
        analytics = db.query(SiteAnalytics).filter(
            SiteAnalytics.deployment_id == deployment.id
        ).first()
    ...
    recent_deployments = db.query(Deployment).filter(
        Deployment.user_id == current_user.id,
        Deployment.created_at >= week_ago
    ).count()
    ...
    return {
        ...
        "success_rate": round((total_views - total_errors) / max(total_views, 1) * 100, 1)
    }
    ...
    for deployment in deployments:
        ...
        deployment_list.append({
            ...
            "analytics": {
                ...
                "last_error_time": analytics.last_error_time.isoformat() if analytics.last_error_time else None
            }
        })
...
async def get_deployment_analytics(
    ...
    analytics = db.query(SiteAnalytics).filter(
        SiteAnalytics.deployment_id == deployment_id
    ).first()
...
async def track_deployment_visit(
    deployment_id: int,
    db: Session = Depends(get_db)
):
    ...
    analytics.unique_visitors += 1  # В реальном приложении нужно отслеживать уникальность
